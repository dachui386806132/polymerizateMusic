...
<script>
import {getAlbumMsg} from "@/api/searchApi";
import {getLikeAlbum} from "@/api/likeApi";
...
export default {
  ...
  methods:{
    ...
    async getMsg(){
      this.loading = true
      let ids = await getLikeAlbum(this.$store.state.user.id)
      this.playlist = []
      this.ids = ids
      for (let i = 0; i < ids.length; i++) {
        if (i>=this.page*10-10 && i<this.page*10){
          let {data:res} = await getAlbumMsg(ids[i])
          this.playlist.push(res.album)
          console.log(res)
        }
      }
      this.loading = false
