<template>
  <div class="music-list-container">
    ...
    <div v-if="userLikeList.length===0">
      <NotFind></NotFind>
    </div>
    <div v-else>
      <!--    歌曲操作按钮-->
      ...
      <div class="mod_songlist">
        ...
        <ul class="songlist_list" >
          ...
          <Songlist v-for="(item,index) in nowMusicList"
                    :itemIndex="startPage * 10 - 10 + index"
                    :songId="item.id"
                    :songName="item.name"
...
export default {
  ...
  methods:{
    ...
    async getLikeNum(){
      this.musicNum = await userLikeNumApi(this.showUserMsg.id)
    },
   async getLike(){
     let {data:res} = await userLikeSongApi(this.showUserMsg.id,this.startPage * 10 - 10)
     let arr = []
     res.data.forEach(item=> arr.push(item.music_id))
     this.userLikeList = arr
    },
    async getMusic(){
     let {data:res} = await getMusicMessageApi(this.userLikeList)
     this.nowMusicList = res.songs
     this.changeLikeList(this.nowMusicList)
     this.loading = false
    },
    ...
    async myPlayAll(){
      ...
      let {data:res} = await userLikeSongAll(userId)
      await this.playAll(res.data)
     },
   async download() {
